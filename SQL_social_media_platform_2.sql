...
SELECT contents, username, photo_id FROM comments c JOIN users u on u.id = c.user_id;


-- 4. Another Quick Join

SELECT contents, url FROM comments c join photos p on p.id = c.photo_id;


-- 4. RELATING RECORDS WITH JOIN 5. Exercise overview 6. 7. 8.

-- 4. RELATING RECORDS WITH JOIN 11. Four Kinds of Join
-- 4. RELATING RECORDS WITH JOIN 12. Each join in practice
...
SELECT url, username FROM photos p LEFT JOIN users u on p.user_id = u.id;

INSERT INTO users (username) VALUES ('Nicole');

SELECT url, username FROM photos p LEFT JOIN users u on p.user_id = u.id;

SELECT url, username FROM photos p RIGHT JOIN users u on p.user_id = u.id;

SELECT url, username FROM photos p FULL JOIN users u on p.user_id = u.id;

-- 4. RELATING RECORDS WITH JOIN 13. Does Order Matter

-- 4. RELATING RECORDS WITH JOIN 18. Where with Join
SELECT url, contents FROM comments c JOIN photos p on p.id = c.photo_id WHERE c.user_id = p.user_id;


-- 4. RELATING RECORDS WITH JOIN 19. Three Way Joins

SELECT url, contents, username FROM comments c
